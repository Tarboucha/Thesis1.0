...
class RobotInfoHandler:
    ...
    def add_robot(self, name, team, team_color, number, host, last_seen, pose=None, vision_pose=None, state=None, maintenance_time_remaining=0.0, maintenance_cycles=0):
        # Create a new Robot message
        robot = self.robot_info.robots.add()
        robot.name = name
        robot.team = team
        robot.team_color = team_color
        robot.number = number
        robot.host = host
        robot.last_seen.sec = last_seen['seconds']
        robot.last_seen.nsec = last_seen.get('nanos', 0)

        # Optional fields
        if pose:
            #robot.pose.timestamp=robot.last_seen
            robot.pose.timestamp.sec = last_seen['seconds']
            robot.pose.timestamp.nsec = last_seen.get('nanos', 0)
            robot.pose.x = pose['x']
            robot.pose.y = pose['y']
            robot.pose.ori = pose['theta']

        if vision_pose:
            robot.vision_pose.x = vision_pose['x']
            robot.vision_pose.y = vision_pose['y']
            robot.vision_pose.theta = vision_pose['theta']

        if state:
            robot.state = state

        robot.maintenance_time_remaining = maintenance_time_remaining
        robot.maintenance_cycles = maintenance_cycles

    def serialize(self):
        # Serialize the RobotInfo message to a binary string
        return self.robot_info.SerializeToString()
    ...
    def process_robot_msg(self,data):
        message = RobotInfo.Robot()
        message.ParseFromString(data)
        return message
