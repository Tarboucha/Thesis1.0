...
public class JSSPAgent : Agent
{
    ...
    private Vector3 task;

    //Dictionaries
    Workstation workstation;
    DeliveryStation delivery;
    ...
    private Dictionary<Collider, DeliveryStation> deliveryColliderDictionary;
    ...
    public void Awake()
    {
        ...
        deliveryColliderDictionary = new Dictionary<Collider, DeliveryStation>();
    ...
    public void Update()
    {
        ...
        if (carrying)
        {
            if (grabbedID > 0)
            {
                ...
                Vector3 destination = prod.GetDestination(grabbedID, inputLocationDictionary);
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        if (!carrying)
        {
            if (action > 0)
            {
                ...
                job = action;
                Vector3 destination = prod.GetDestination(job, inputLocationDictionary);
                destination.y = init_pos.y;
                agent.destination = destination;
    ...
    private void GetLocations(Transform parent)
    {
        for (int i = 0; i < parent.childCount; i++)
        {
            ...
            if (child.CompareTag("basestation") || child.CompareTag("ringstation") || child.CompareTag("capstation")
